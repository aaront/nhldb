...
import nhldb.db as db
import nhldb.models as models
...
def _load_team(session, row):
    team = dict(id=int(row['']), name=row['team'], full_name=row['TeamName'],
...
def _load_game(session, row):
    ...
    game = dict(id=id, season=season, status=int(row['status']), away_team=awayteam.id, home_team=hometeam.id,
